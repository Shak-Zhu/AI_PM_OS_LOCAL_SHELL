# ai-pm-os Skill 操作规则

- 版本：v0.4
- 状态：Active
- 最后更新：2026-06-22

每次执行必须完成 Memory Boot、识别意图、读取必要文件、生成制品、验证
门检查、更新项目记忆和输出下一步。Skill 不自研 parser/OCR；它处理当前
Cursor/Codex 上下文可读材料。正式更新后主动同步 JSON；P0 不使用后台
监听。生成 Pending Updates 后必须立即请求用户确认。

## 1. 强制执行循环（memory-and-recovery.md REQUIRED_MEMORY_BOOT_FILES）

执行顺序统一为：Memory Boot → Cooper Helper Bootstrap → Intent Routing。

1. **Memory Boot**：`ai-pm-os/references/memory-and-recovery.md` 定义的六层信息源 +
   严格读取顺序；`REQUIRED_MEMORY_BOOT_FILES` 定义 Global Rules 层 3 文件 + PM Memory 层 6 文件。
2. **Cooper Helper Bootstrap**（Memory Boot 后、正常意图路由前）：
   在 `ai-pm-os/references/cooper-helper-bootstrap.md` 状态机控制下，
   运行 `node ai-pm-os/scripts/bootstrap-cooper-helper.js`。
   - 已安装 → 永久跳过。
   - 未安装且无 deferred/unavailable → 尝试固定安装命令。
   - deferred/unavailable → 非阻塞，不自动重试。
   - 明确"重试 Cooper 安装"时才加 `--retry`。
   - bootstrap 非 0 或 unavailable 不阻断 Skill 执行。
3. **Skill 入口**：`ai-pm-os/SKILL.md` + 9 个 references + 146 个场景（含 8 个 Remote Intake 场景）。
4. **意图路由**：`ai-pm-os/references/router.md`。
5. **框架选择**：`ai-pm-os/references/framework-matrix.md`，显式标注主框架
   与辅助框架。
6. **事实层级**：`ai-pm-os/references/fact-layers.md`，输出必须标注 `Fact-Layer`。
7. **稳定性**：`ai-pm-os/references/stability-rules.md`，处理重复 / 冲突 /
   恢复 / 脏工作树 / Critical Output Contract 失败关闭。
8. **执行完整性**：`ai-pm-os/references/execution-integrity.md`（WP-005），执行身份模型、
   状态机、四类重入、幂等重放、at-most-once PU、部分失败恢复、Markdown→JSON 恢复方向。
9. **敏捷交付**：`ai-pm-os/references/agile-delivery-rules.md`，Scrum/Kanban/Hybrid
   行为规则与语义不变量。
10. **场景校验**：`ai-pm-os/scenarios/scenarios.md`，匹配 ≥1 个基准场景（含执行完整性场景）。
11. **契约规范**：`ai-pm-os/references/runtime-compliance-contracts.md`，定义审批转达契约字段、门禁步骤与禁止状态。
12. **失败升级**：见 SKILL.md §5；不得静默重试或越权。

## 2. 验证脚本

- `node scripts/validate-skill.js` 必退出 0。
- 删除 / 改名 `ai-pm-os/SKILL.md` 中任一必需能力标签 → 退出 1。
- 场景数 < 50 → 退出 1。
- 出现平台绝对路径 → 退出 1。
- SI-01~SI-14 任一不满足 → 退出 1。

## 3. 关键边界

- 不得绕过 Pending Updates 直接修改关键正式文件。
- 不得在 Scope Baseline 未批准前进入 Sprint 或拆正式 WBS。
- 不得把会议 transcript 仅作文本总结；必须提取 Action / Decision /
  Risk / Issue / Change / Pending Update。
- 不得编造无法读取的输入内容；不可读输入进入 `received-but-unreadable`。
- 不得在生成 Pending Updates 后跳过对话确认。
- 不得在脏工作树下自动 `git add` / `git commit` / `git push`。
- 不得把"一键复制""简洁""赶快"解释为 path-only 授权。
- 不得在 Pre-send Compliance Gate FAIL 时输出 `issued` / `accepted` / `complete` / `done` / `finished`。
- 不得在缺字段、缺渠道或授权不明时输出错误成功状态。

## 4. 项目实例边界

- Skill 只使用当前项目壳内的正式项目文件恢复和维护项目事实。
- 不得假设存在任何 AI PM OS 产品开发目录、私有控制空间或外部治理文件。
- 软件交付项目使用 WBS/Action 管理，不涉及特殊委派流程。
