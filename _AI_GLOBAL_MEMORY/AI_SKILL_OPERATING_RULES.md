...
2. **Skill 入口**：`ai-pm-os/SKILL.md` + 9 个 references + 60 个场景。
...
7. **执行完整性**：`ai-pm-os/references/execution-integrity.md`（WP-005），执行身份模型、
   状态机、四类重入、幂等重放、at-most-once PU、部分失败恢复、Markdown→JSON 恢复方向。
8. **敏捷交付**：`ai-pm-os/references/agile-delivery-rules.md`，Scrum/Kanban/Hybrid
   行为规则与语义不变量。
9. **场景校验**：`ai-pm-os/scenarios/scenarios.md`，匹配 ≥1 个基准场景（含 SC-COC-01~08 关键输出契约场景 + SC-EI-01~10 执行完整性场景）。
10. **Critical Output Contract**：`ai-pm-os/references/runtime-compliance-contracts.md`，
    6 类契约、10 字段、Pre-send Compliance Gate 8 步、双输出事务与错误成功状态禁止。
11. **失败升级**：见 SKILL.md §5；不得静默重试或越权。
...
- Coder Work Package 仅在 Human Owner 明确启用 AI Coder 委派的软件交付场景中使用。
- 普通项目默认使用 WBS、Action、里程碑、Backlog 和 Sprint 管理执行。
