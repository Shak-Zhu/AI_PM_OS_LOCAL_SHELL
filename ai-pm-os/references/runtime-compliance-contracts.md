...
`GOVERNANCE_ROOT` 是本注册表与所有契约运行时定位**开发期控制空间**（PM 治理工作包与 QC 报告）的唯一入口；其值必须可由运行环境在**不依赖任何宿主操作系统环境变量**的前提下确定性确定。
...
- 默认治理根是**产品壳内的项目相对路径**，不等于、不包含 `_DEV_PROJECT_CONTROL/`。
- **开发期** `_DEV_PROJECT_CONTROL/` 仅可通过优先级 1（显式运行时注入）或优先级 2（`--governance-root` 参数）覆盖，不得成为产品默认治理根。
- 不得在源码、注册表或场景中硬编码 `_DEV_PROJECT_CONTROL/` 作为可交付壳的默认路径；必须通过 `${GOVERNANCE_ROOT}` 占位符访问。
...
- `GOVERNANCE_ROOT` 解析按 §0.5 优先级与合法性规则；缺省值为项目相对路径 `01_PM_DOCUMENTS/AI_PM_GOVERNANCE/`，等于、不包含 `_DEV_PROJECT_CONTROL/`；解析失败 → fail-closed
