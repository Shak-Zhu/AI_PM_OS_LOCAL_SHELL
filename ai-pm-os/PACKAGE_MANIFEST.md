...
| `references/communication-and-reporting-rules.md` | BRIEFING/MEETING/TODO/REPORT_DAILY/REPORT_PERIODIC/DASHBOARD_SYNC 六个 P0 专业工作流行为契约，含 9 字段、质量检查、事实来源禁止编造规则 | 无 |
...
| 文件 | 用途 | 包外依赖 |
|---|---|---|
| `scenarios/scenarios.md` | 102 个结构化 Given/When/Then/Allow/Forbid/Evidence 场景（60 原 + 10 WP-006 新增 + 10 WP-007 新增 + 10 WP-008 新增 + 12 WP-009 新增） | 无 |

### 1.4 包内验证脚本（scripts/）

| 文件 | 用途 | 说明 |
|---|---|---|
| `scripts/validate-skill.js` | SI-01~SI-48 机器可验证规则；自包含运行（无外部包依赖）；支持隔离模式（无宿主文件时跳过 AGENTS.md/_AI_GLOBAL_MEMORY/ 检查） | 唯一验证实现 |
...
根目录 `scripts/` 下的文件（如 `check-pollution.js`）是**仓库级 QA 适配器**，不是 Skill 运行时必需文件。它们：

- 扫描整个产品目录（包括 `ai-pm-os/`）
- 使用产品级污染模式（如 `PU-CHG-###`）
- 需要访问 `_DEV_PROJECT_CONTROL/` 等仓库治理文件

安装说明：
- **本地复制安装**：用户复制整个项目壳（含根目录 `scripts/`）后，两类脚本均可使用。
- **Git URL 安装（仅 `ai-pm-os/`）**：`scripts/check-pollution.js` 等仓库 QA 适配器需要用户额外复制根目录 `scripts/`。
