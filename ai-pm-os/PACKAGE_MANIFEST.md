...
| 文件 | 用途 | 包外依赖 |
|---|---|---|
| `scenarios/scenarios.md` | 60 个结构化 Given/When/Then/Allow/Forbid/Evidence 场景 | 无 |

### 1.4 包内验证脚本（scripts/）

| 文件 | 用途 | 说明 |
|---|---|---|
| `scripts/validate-skill.js` | SI-01~SI-20 机器可验证规则；自包含运行（无外部包依赖）；支持隔离模式（无宿主文件时跳过 AGENTS.md/_AI_GLOBAL_MEMORY/ 检查） | 唯一验证实现 |
